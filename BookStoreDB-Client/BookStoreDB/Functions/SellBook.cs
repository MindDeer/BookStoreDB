...
namespace BookStoreDB.Functions
{
    public partial class SellBook : Form
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("p_listsellcopy", MainForm.conn);
        ...
        {
            SqlCommand cmd = new SqlCommand("p_clearsellcopy", MainForm.conn);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.ExecuteNonQuery();  //执行零时存储表的清空
            cmd = new SqlCommand("p_listsellcopy", MainForm.conn);
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        {
            SqlCommand cmd = new SqlCommand("p_addsellcopy",MainForm.conn);
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            SqlCommand cmd1 = new SqlCommand("p_clearsellcopy", MainForm.conn);
        ...
        {
            SqlCommand cmd = new SqlCommand("p_updatesellcopy", MainForm.conn);
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        private void button删除_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("p_deletesellcopy", MainForm.conn);
            cmd.CommandType = CommandType.StoredProcedure;            
            ...
            for (int i = 0; i < dataGridView1.RowCount; i++)
            {
                if (dataGridView1.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True") //这里判断复选框是否选中
                {
                    ...
                    cmd2 = new SqlCommand("p_listsellcopy", MainForm.conn);
        ...
        {            
            SqlCommand cmd = new SqlCommand("p_insertsellcopy", MainForm.conn);
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            SqlCommand cmd2 = new SqlCommand("p_listsellcopy", MainForm.conn);
            cmd2.CommandType = CommandType.StoredProcedure;
            try
            {
                cmd2.ExecuteNonQuery();
        ...
        private void ShowTable(DataGridView DG, SqlCommand cmd)
        {
            ...
            DG.Columns["Column1"].DataPropertyName = dtb.Columns["图书副本号"].ToString();
            DG.Columns["Column2"].DataPropertyName = dtb.Columns["图书名称"].ToString();
            DG.Columns["Column3"].DataPropertyName = dtb.Columns["ISBN编号"].ToString();
            DG.Columns["Column4"].DataPropertyName = dtb.Columns["作者"].ToString();
            DG.Columns["Column5"].DataPropertyName = dtb.Columns["出版社"].ToString();
        ...
        {
            double sum=0;
            SqlCommand cmd = new SqlCommand("p_sumsellcopy", MainForm.conn);
            cmd.CommandType = CommandType.StoredProcedure;
