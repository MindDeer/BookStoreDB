...
namespace BookStoreDB.Functions
{
    public partial class InsertBook : Form
    {
        ...
        public InsertBook()
        {
            ...
            cmd = new SqlCommand("p_listTS", MainForm.conn);
        ...
        {            
            ...
            {
                SqlCommand cmd = new SqlCommand("p_insertBook", MainForm.conn);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                cmd.Parameters["@tsId"].Value = int.Parse(row.Cells[1].Value.ToString().Trim());
                ...
                cmd.Parameters["@money"].Value = Decimal.Parse(row.Cells[7].Value.ToString());
                cmd.Parameters["@cbdate"].Value = DateTime.Parse(row.Cells[8].Value.ToString());

                //try
                //{
                    cmd.ExecuteNonQuery();  //执行存储过程调用
                    label2.Text = "提示：图书信息录入成功！" ;
                //}
                //catch (Exception ev)
                //{
            ...
            {
                SqlCommand cmd = new SqlCommand("p_updateBook", MainForm.conn);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                for (int i = 0; i < dataGridView1.RowCount; i++)
                {
                    if (dataGridView1.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")//这里判断复选框是否选中
                    {
                        ...
                        cmd.Parameters["@tsId"].Value = int.Parse(row.Cells[1].Value.ToString());
                        cmd.Parameters["@tsmc"].Value = row.Cells[2].Value.ToString();
                        cmd.Parameters["@type"].Value = row.Cells[3].Value.ToString();
                        cmd.Parameters["@code"].Value = row.Cells[4].Value.ToString();
                        cmd.Parameters["@author"].Value = row.Cells[5].Value.ToString();
                        ...
                        {
                            cmd.ExecuteNonQuery();  //执行存储过程调用
                            label2.Text = "提示：成功修改 " + count + " 本图书信息！";
                        }
                        catch (Exception ev)
                        {
                            label2.Text = "提示：服务器异常";
            ...
            else if (radioButton3.Checked)
            {
                ...
                for (int i = 0; i < dataGridView1.RowCount; i++)
                {
                    if (dataGridView1.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")//这里判断复选框是否选中
                    {
                        ...
                        {
                            cmd.ExecuteNonQuery();  //执行存储过程调用
                            label2.Text = "提示：成功删除 "+count+" 本图书信息！";
                        }
                        catch (Exception ev)
                        {
                            label2.Text = "提示：服务器异常";
        ...
        private void ShowTable(DataGridView DG, SqlCommand cmd)
        {
            ...
            DG.Columns["Column8"].DataPropertyName = dtb.Columns["出版日期"].ToString();
