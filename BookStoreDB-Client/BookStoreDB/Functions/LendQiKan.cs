...
{
    public partial class LendQiKan : Form
    {
        public LendQiKan()
        {
            InitializeComponent();
        ...
        private void buttonOK_Click(object sender, EventArgs e)
        {
            ...
            if (rbAll.Checked)
            {
                cmd = new SqlCommand("p_Alllist2", MainForm.conn);
            ...
            {
                cmd = new SqlCommand("p_dzlist2", MainForm.conn);
                cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                cmd = new SqlCommand("p_qklist2", MainForm.conn);
                cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                cmd = new SqlCommand("p_datelist2", MainForm.conn);
                cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                cmd = new SqlCommand("p_adminlist2", MainForm.conn);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        private void ShowTable(DataGridView DG, SqlCommand cmd)
        {
            ...
            DG.Columns["Column1"].DataPropertyName = dtb.Columns["期刊名"].ToString();
            DG.Columns["Column2"].DataPropertyName = dtb.Columns["副本条码"].ToString();
            DG.Columns["Column3"].DataPropertyName = dtb.Columns["借阅日期"].ToString();
            DG.Columns["Column4"].DataPropertyName = dtb.Columns["归还日期"].ToString();
